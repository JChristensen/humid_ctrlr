...
void setup(void)
{
    ...
    delay(1000);
    LCD.clear();
    if ( !XB.begin(Serial) ) XB.mcuReset(RESET_DELAY);    //reset if XBee initialization fails
    Humidifier.begin();
    hbLED.begin();
}
...
void loop(void)
{
    ...
    if ( haveData || stale != lastStale )
    {
        ...
        Serial << millis() << F(" run fsm ");
        if ( hState != last_hState ) Serial << F("hState ") << hState;
        Serial << endl;
    }
    ...
    {
        LCD.clear(); LCD << F("Humidifier "); LCD.setCursor(11, 0);
